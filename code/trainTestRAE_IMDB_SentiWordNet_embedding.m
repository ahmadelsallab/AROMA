...
params.CVNUM = 10;
preProFile = ['../data/IMDB_SentiWordNet_Embedding/RTData_CV' num2str(params.CVNUM) '.mat'];

% read in polarity dataset
if ~exist(preProFile,'file')
    read_rtPolarity_IMDB_SentiWordNet_embedding;
else
    
    global bKnownParses;
    if(bKnownParses)
        load(preProFile, 'labels','train_ind','test_ind', 'cv_ind','We2','allSNum','test_nums', 'allKids');
    else
...
%%%%%%%%%%%%%%%%%%%%%%
theta = initializeParameters_SentiWordNet_embedding(params.embedding_size, params.embedding_size, cat_size);
...
if params.trainModel
    ...
    save(['../output/IMDB_SentiWordNet_Embedding/savedParams_CV' num2str(params.CVNUM) '.mat'],'opttheta','params','options');
    classifyWithRAE_ATB
    ...
    load('../output/IMDB_SentiWordNet_Embedding/optParams_RT_CV1.mat')
