...
% node and word size
load('../data/ATB_ArSenL_We/final_net_We.mat');
We = NM_strNetParams.cWeights{1};
load('../data/ATB_ArSenL_We/final_net_ArSenL_embedding.mat');
We = [We NM_strNetParams.cWeights{1}];
...
params.CVNUM = 1;
preProFile = ['../data/ATB_ArSenL_We/RTData_CV' num2str(params.CVNUM) '.mat'];

% read in polarity dataset
if ~exist(preProFile,'file')
    read_rtPolarity_ATB_ArSenL_We
else
...
%%%%%%%%%%%%%%%%%%%%%%
theta = initializeParameters_ArSenL_We(params.embedding_size, params.embedding_size, cat_size);
...
if params.trainModel
    ...
    save(['../output/ATB_ArSenL_We/savedParams_CV' num2str(params.CVNUM) '.mat'],'opttheta','params','options');
    classifyWithRAE_ATB
    ...
    load('../output/ATB_ArSenL_We/optParams_RT_CV1.mat')
